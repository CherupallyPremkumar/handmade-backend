Feature: Platform Admin Dashboard Queries
  ...
  Scenario: View Unified Marketplace Stats
    ...
    And the REST response key "payload.list[?(@.row.metric_type == 'SELLERS' && @.row.metric_group == 'ALL')].row.metric_count" is 10
    And the REST response key "payload.list[?(@.row.metric_type == 'PRODUCTS' && @.row.metric_group == 'ALL')].row.metric_count" is 13
