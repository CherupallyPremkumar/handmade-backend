 ...
 * GOVERNANCE: Only ONE policy per (country, seller_type) can be ACTIVE at a
 * time
 * 
 * State Machine Flow:
 ...
 */
@Entity
@Table(name = "payout_policies", uniqueConstraints = {
        @UniqueConstraint(columnNames = { "country_code", "seller_type", "version" })
}, indexes = {
        @Index(name = "idx_active_payout_policies", columnList = "country_code, seller_type, state_id, effective_from"),
        @Index(name = "idx_payout_policy_version", columnList = "version")
})
