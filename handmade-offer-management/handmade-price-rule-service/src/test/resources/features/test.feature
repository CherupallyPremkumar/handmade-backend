Feature: Testcase ID 
Tests the pricerule Workflow Service using a REST client. Pricerule service exists and is under test.
...
And store "$.payload.mutatedEntity.currentState.stateId" from response to "finalState"
Scenario: Send the deactivate event to the pricerule with comments
