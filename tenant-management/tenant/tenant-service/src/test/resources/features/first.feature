Feature: Tests the tenant Workflow Service using a REST client. This is done only for the
first testcase. Tenant service exists and is under test.
...
And store "$.payload.mutatedEntity.currentState.stateId" from response to "finalState"
 ...
 Scenario: Send the reactivate event to the tenant with comments
 Given that "comment" equals "Comment for reactivate"
...
Send cc event on the tenant with comments. This should fail since the mandatory activities
have not been completed.
Given that "terminalState" equals "__TERMINAL_STATE__"
And that config strategy is "tenantConfigProvider" with prefix "Tenant"
...
Scenario: Perform mandatory activity (a1) on the  tenant with comments
...
Scenario: Perform mandatory activity (a2) on the  tenant with comments
...
Scenario: Perform mandatory activity (cc) on the  tenant with comments
...
Scenario: Send an invalid event to tenant . This will err out.
When I construct a REST request with authorization header in realm "tenant0" for user "t0-premium" and password "t0-premium"
And I construct a REST request with header "x-chenile-tenant-id" and value "tenant0"
When I PATCH a REST request to URL "/tenant/${id}/invalid" with payload
"""json
{
