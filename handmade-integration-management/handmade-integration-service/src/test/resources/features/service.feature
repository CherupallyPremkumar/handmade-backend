Feature: Tests the integration Service using a REST client.
 
  Scenario: Save the integration first.
    ...
    And I POST a REST request to URL "/integration" with payload
    """
    {
  ...
  Scenario: Retrieve the saved integration
    Given that "entity" equals "integration"
    And I construct a REST request with header "x-chenile-tenant-id" and value "tenant0"
    And I GET a REST request to URL "/${entity}/${id}"
    Then success is true
    And the REST response key "id" is "${id}"

  Scenario: Save a integration using an ID that already is determined
