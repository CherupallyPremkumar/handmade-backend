Feature: Testcase ID 
Tests the productanswer Workflow Service using a REST client. Productanswer service exists and is under test.
...
And the REST response key "mutatedEntity.currentState.stateId" is "UNDER_REVIEW"
And store "$.payload.mutatedEntity.currentState.stateId" from response to "finalState"
Scenario: Send the reject event to the productanswer with comments
...
Scenario: Send the publish event to the productanswer with comments
