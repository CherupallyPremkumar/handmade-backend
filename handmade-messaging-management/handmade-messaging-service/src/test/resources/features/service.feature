Feature: Tests the messaging Service using a REST client.
 
  Scenario: Save the messaging first.
    ...
    And I POST a REST request to URL "/messaging" with payload
    """
    {
  ...
  Scenario: Retrieve the saved messaging
    Given that "entity" equals "messaging"
    And I construct a REST request with header "x-chenile-tenant-id" and value "tenant0"
    And I GET a REST request to URL "/${entity}/${id}"
    Then success is true
    And the REST response key "id" is "${id}"

  Scenario: Save a messaging using an ID that already is determined
