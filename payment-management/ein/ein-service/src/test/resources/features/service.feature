Feature: Tests the ein Service using a REST client.
 
  Scenario: Save the ein first.
    ...
    And I POST a REST request to URL "/ein" with payload
    """
    {
  ...
  Scenario: Retrieve the saved ein
    Given that "entity" equals "ein"
    When I construct a REST request with authorization header in realm "tenant0" for user "t0-premium" and password "t0-premium"
  ...
  Scenario: Save a ein using an ID that already is determined
