Feature: Tests the gstin Service using a REST client.
 
  Scenario: Save the gstin first.
    ...
    And I POST a REST request to URL "/gstin" with payload
    """
    {
  ...
  Scenario: Retrieve the saved gstin
    Given that "entity" equals "gstin"
    When I construct a REST request with authorization header in realm "tenant0" for user "t0-premium" and password "t0-premium"
  ...
  Scenario: Save a gstin using an ID that already is determined
