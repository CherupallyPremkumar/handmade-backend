...
'
tmpfile=/tmp/tmp.$$
echo "Creating reconciliation with payload |$json1| "
curl -X POST -d "${json1}" -H 'Content-Type: application/json' -H 'x-chenile-eid: E1' -H 'x-chenile-tenant-id: t1' localhost:8080/reconciliation > $tmpfile
cat $tmpfile | jq
id=$(cat $tmpfile | jq ".payload.id" | tr -d '"')
echo "Obtained ID = $id"
rm -f $tmpfile

# execute  "-X POST -d '${json_create}' -H 'Content-Type: application/json' localhost:8080/reconciliation"

execute  "-X GET  -H 'Content-Type: application/json' localhost:8080/reconciliation/${id}"
