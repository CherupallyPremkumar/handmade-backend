Feature: Tests the stripe Service using a REST client.
 
  Scenario: Save the stripe first.
    ...
    And I POST a REST request to URL "/stripe" with payload
    """
    {
  ...
  Scenario: Retrieve the saved stripe
    Given that "entity" equals "stripe"
    When I construct a REST request with authorization header in realm "tenant0" for user "t0-premium" and password "t0-premium"
  ...
  Scenario: Save a stripe using an ID that already is determined
