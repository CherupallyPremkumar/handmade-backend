Feature: Tests the cash Service using a REST client.
 
  Scenario: Save the cash first.
    ...
    And I POST a REST request to URL "/cash" with payload
    """
    {
  ...
  Scenario: Retrieve the saved cash
    Given that "entity" equals "cash"
    And I construct a REST request with header "x-chenile-tenant-id" and value "tenant0"
    And I GET a REST request to URL "/${entity}/${id}"
    Then success is true
    And the REST response key "id" is "${id}"

  Scenario: Save a cash using an ID that already is determined
