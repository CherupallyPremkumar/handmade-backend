Feature: Tests the variant Workflow Service using a REST client. This is done only for the
first testcase. Variant service exists and is under test.
...
Send cc event on the variant with comments. This should fail since the mandatory activities
have not been completed.
Given that "terminalState" equals "__TERMINAL_STATE__"
And that config strategy is "variantConfigProvider" with prefix "Variant"
...
Scenario: Perform mandatory activity (a1) on the  variant with comments
...
Scenario: Perform mandatory activity (a2) on the  variant with comments
...
Scenario: Perform mandatory activity (cc) on the  variant with comments
...
Scenario: Send an invalid event to variant . This will err out.
When I construct a REST request with authorization header in realm "tenant0" for user "t0-premium" and password "t0-premium"
And I construct a REST request with header "x-chenile-tenant-id" and value "tenant0"
When I PATCH a REST request to URL "/variant/${id}/invalid" with payload
"""json
{
