Feature: Tests the paymentauthorization Workflow Service using a REST client. This is done only for the
first testcase. Paymentauthorization service exists and is under test.
It helps to create a paymentauthorization and manages the state of the paymentauthorization as documented in states xml
Scenario: Create a new paymentauthorization
Given that "flowName" equals "paymentAuthorizationFlow"
And that "initialState" equals "AUTHORIZED"
When I POST a REST request to URL "/paymentauthorization" with payload
"""json
{
    "description": "Description"
}
"""
Then the REST response contains key "mutatedEntity"
And store "$.payload.mutatedEntity.id" from response to "id"
And the REST response key "mutatedEntity.currentState.stateId" is "${initialState}"
And store "$.payload.mutatedEntity.currentState.stateId" from response to "currentState"
And the REST response key "mutatedEntity.description" is "Description"

Scenario: Retrieve the paymentauthorization that just got created
When I GET a REST request to URL "/paymentauthorization/${id}"
Then the REST response contains key "mutatedEntity"
And the REST response key "mutatedEntity.id" is "${id}"
And the REST response key "mutatedEntity.currentState.stateId" is "${currentState}"

 Scenario: Send the void event to the paymentauthorization with comments
 Given that "comment" equals "Comment for void"
 And that "event" equals "void"
When I PATCH a REST request to URL "/paymentauthorization/${id}/${event}" with payload
"""json
{
    "comment": "${comment}"
}
"""
Then the REST response contains key "mutatedEntity"
And the REST response key "mutatedEntity.id" is "${id}"
And the REST response key "mutatedEntity.currentState.stateId" is "VOIDED"
And store "$.payload.mutatedEntity.currentState.stateId" from response to "finalState"



Scenario: Send an invalid event to paymentauthorization . This will err out.
When I PATCH a REST request to URL "/paymentauthorization/${id}/invalid" with payload
"""json
{
    "comment": "invalid stuff"
}
"""
Then the REST response does not contain key "mutatedEntity"
And the http status code is 422

