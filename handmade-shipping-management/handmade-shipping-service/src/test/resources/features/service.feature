Feature: Tests the shipping Service using a REST client.
 
  Scenario: Save the shipping first.
    ...
    And I POST a REST request to URL "/shipping" with payload
    """
    {
  ...
  Scenario: Retrieve the saved shipping
    Given that "entity" equals "shipping"
    And I construct a REST request with header "x-chenile-tenant-id" and value "tenant0"
    And I GET a REST request to URL "/${entity}/${id}"
    Then success is true
    And the REST response key "id" is "${id}"

  Scenario: Save a shipping using an ID that already is determined
