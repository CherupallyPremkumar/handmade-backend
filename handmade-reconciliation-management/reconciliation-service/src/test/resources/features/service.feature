Feature: Tests the reconciliation Service using a REST client.
 
  Scenario: Save the reconciliation first.
    ...
    And I POST a REST request to URL "/reconciliation" with payload
    """
    {
  ...
  Scenario: Retrieve the saved reconciliation
    Given that "entity" equals "reconciliation"
    When I construct a REST request with authorization header in realm "tenant0" for user "t0-premium" and password "t0-premium"
  ...
  Scenario: Save a reconciliation using an ID that already is determined
