package com.handmade.ecommerce.variant.service;
...
public class VariantServiceImpl extends StateEntityServiceImpl<Variant> implements VaraiantService {
    ...
    private void makeObject(Variant variant, CreateVariantCommand command) {
        ...
        if (command.getShippingProfiles() != null && !command.getShippingProfiles().isEmpty()) {
            List<com.handmade.ecommerce.variant.model.ShippingProfile> entityProfiles = new ArrayList<>();
            for (com.handmade.ecommerce.command.CreateShippingProfileCommand cmdProfile : command
                    .getShippingProfiles()) {
                com.handmade.ecommerce.variant.model.ShippingProfile entityProfile = new com.handmade.ecommerce.variant.model.ShippingProfile();

                // Map profile type
                if (cmdProfile.getProfileType() != null) {
                    entityProfile.setProfileType(
                            com.handmade.ecommerce.variant.model.ShippingProfile.ProfileType.valueOf(
        ...
        if (command.getVariantAttributes() != null) {
            ...
            for (com.handmade.ecommerce.command.VariantAttributeDTO cmdAttr : command.getVariantAttributes()) {
