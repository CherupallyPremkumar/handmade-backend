...
And the REST response key "mutatedEntity.currentState.stateId" is "${currentState}"
 ...
 Scenario: Send the addVariant event to the product with comments
 Given that "comment" equals "Comment for addVariant"
 And that "event" equals "addVariant"
When I construct a REST request with authorization header in realm "tenant0" for user "t0-premium" and password "t0-premium"
And I construct a REST request with header "x-chenile-tenant-id" and value "tenant0"
...
And store "$.payload.mutatedEntity.currentState.stateId" from response to "finalState"
 ...
 Scenario: Send the archive event to the product with comments
