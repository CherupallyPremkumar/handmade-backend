...
Then the REST response contains key "mutatedEntity"
And the REST response key "mutatedEntity.id" is "${id}"
And the REST response key "mutatedEntity.currentState.stateId" is "PENDING_SHIPMENT_FULFILLMENT"
And store "$.payload.mutatedEntity.currentState.stateId" from response to "finalState"

Scenario: Send the itemPack event to the orderline with comments
Given that "comment" equals "Comment for itemPack"
And that "event" equals "itemPack"
When I construct a REST request with authorization header in realm "tenant0" for user "t0-premium" and password "t0-premium"
And I construct a REST request with header "x-chenile-tenant-id" and value "tenant0"
When I PATCH a REST request to URL "/orderline/${id}/${event}" with payload
"""json
