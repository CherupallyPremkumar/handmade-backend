...
And the REST response key "mutatedEntity.currentState.stateId" is "${currentState}"
 ...
 Scenario: Send the reload event to the orderline with comments
 Given that "comment" equals "Comment for reload"
 And that "event" equals "reload"
When I construct a REST request with authorization header in realm "tenant0" for user "t0-premium" and password "t0-premium"
And I construct a REST request with header "x-chenile-tenant-id" and value "tenant0"
...
Then the REST response contains key "mutatedEntity"
And the REST response key "mutatedEntity.id" is "${id}"
And the REST response key "mutatedEntity.currentState.stateId" is "VOID"
...
Send cc event on the orderline with comments. This should fail since the mandatory activities
have not been completed.
Given that "terminalState" equals "__TERMINAL_STATE__"
And that config strategy is "orderlineConfigProvider" with prefix "Orderline"
...
Scenario: Perform mandatory activity (a1) on the  orderline with comments
...
Scenario: Perform mandatory activity (a2) on the  orderline with comments
...
Scenario: Perform mandatory activity (cc) on the  orderline with comments
