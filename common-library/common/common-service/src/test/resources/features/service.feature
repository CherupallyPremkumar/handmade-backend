Feature: Tests the common Service using a REST client.
  ...
  Scenario: Save the common first.
    ...
    And I POST a REST request to URL "/common" with payload
    """
    {
  ...
  Scenario: Retrieve the saved common
    Given that "entity" equals "common"
    And I construct a REST request with header "x-chenile-tenant-id" and value "tenant0"
    And I GET a REST request to URL "/${entity}/${id}"
    Then success is true
    And the REST response key "id" is "${id}"

  Scenario: Save a common using an ID that already is determined
