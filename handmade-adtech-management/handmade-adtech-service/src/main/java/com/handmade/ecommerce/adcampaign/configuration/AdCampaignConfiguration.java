...
public class AdCampaignConfiguration {
    ...
    @Bean BeanFactoryAdapter adCampaignBeanFactoryAdapter() {
		return new SpringBeanFactoryAdapter();
	}
	
	@Bean STMFlowStoreImpl adCampaignFlowStore(
	...
	@Bean @Autowired STM<AdCampaign> adCampaignEntityStm(@Qualifier("adCampaignFlowStore") STMFlowStoreImpl stmFlowStore) throws Exception{
		STMImpl<AdCampaign> stm = new STMImpl<>();
		stm.setStmFlowStore(stmFlowStore);
		return stm;
	}
	
	@Bean @Autowired STMActionsInfoProvider adCampaignActionsInfoProvider(@Qualifier("adCampaignFlowStore") STMFlowStoreImpl stmFlowStore) {
		STMActionsInfoProvider provider =  new STMActionsInfoProvider(stmFlowStore);
        WorkflowRegistry.addSTMActionsInfoProvider("adCampaign",provider);
        return provider;
	}
	
	@Bean EntityStore<AdCampaign> adCampaignEntityStore() {
		return new AdCampaignEntityStore();
	}
	
	@Bean @Autowired StateEntityServiceImpl<AdCampaign> _adCampaignStateEntityService_(
	...
	@Bean @Autowired GenericEntryAction<AdCampaign> adCampaignEntryAction(@Qualifier("adCampaignEntityStore") EntityStore<AdCampaign> entityStore,
			@Qualifier("adCampaignActionsInfoProvider") STMActionsInfoProvider adCampaignInfoProvider,
            @Qualifier("adCampaignFlowStore") STMFlowStoreImpl stmFlowStore){
        GenericEntryAction<AdCampaign> entryAction =  new GenericEntryAction<AdCampaign>(entityStore,adCampaignInfoProvider);
        stmFlowStore.setEntryAction(entryAction);
        return entryAction;
	}
	
	@Bean GenericExitAction<AdCampaign> adCampaignExitAction(@Qualifier("adCampaignFlowStore") STMFlowStoreImpl stmFlowStore){
    ...
    @Bean STMTransitionAction<AdCampaign> defaultadCampaignSTMTransitionAction() {
    ...
    @Bean @Autowired StmBodyTypeSelector adCampaignBodyTypeSelector(
    @Qualifier("adCampaignActionsInfoProvider") STMActionsInfoProvider adCampaignInfoProvider,
    ...
    @Bean @Autowired STMTransitionAction<AdCampaign> adCampaignBaseTransitionAction(
        @Qualifier("adCampaignTransitionActionResolver") STMTransitionActionResolver stmTransitionActionResolver){
    ...
    // them into the STM use the convention of "adCampaign" + eventId for the method name. (adCampaign is the
    ...
    @Bean EndAdCampaignAction endAdCampaignAction(){
