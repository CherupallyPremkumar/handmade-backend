Feature: Tests the compliancepolicy Workflow Service using a REST client. This is done only for the
first testcase. Compliancepolicy service exists and is under test.
...
And store "$.payload.mutatedEntity.currentState.stateId" from response to "finalState"
 ...
 Scenario: Send the suspend event to the compliancepolicy with comments
...
Scenario: Send an invalid event to compliancepolicy . This will err out.
When I PATCH a REST request to URL "/compliancepolicy/${id}/invalid" with payload
"""json
{
