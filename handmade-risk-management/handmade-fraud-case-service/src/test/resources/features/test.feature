Feature: Testcase ID 
Tests the fraudcase Workflow Service using a REST client. Fraudcase service exists and is under test.
...
And store "$.payload.mutatedEntity.currentState.stateId" from response to "finalState"
Scenario: Send the escalate event to the fraudcase with comments
