Feature: Tests the orchestrator Service using a REST client.
 
  Scenario: Save the orchestrator first.
    ...
    And I POST a REST request to URL "/orchestrator" with payload
    """
    {
  ...
  Scenario: Retrieve the saved orchestrator
    Given that "entity" equals "orchestrator"
    When I construct a REST request with authorization header in realm "tenant0" for user "t0-premium" and password "t0-premium"
  ...
  Scenario: Save a orchestrator using an ID that already is determined
