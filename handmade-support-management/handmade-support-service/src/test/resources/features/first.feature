Feature: Tests the support Workflow Service using a REST client. This is done only for the
first testcase. Support service exists and is under test.
...
And store "$.payload.mutatedEntity.currentState.stateId" from response to "finalState"
 ...
 Scenario: Send the reopen event to the support with comments
 Given that "comment" equals "Comment for reopen"
 And that "event" equals "reopen"
When I PATCH a REST request to URL "/support/${id}/${event}" with payload
"""json
{
    "comment": "${comment}"
...
Scenario: Send an invalid event to support . This will err out.
When I PATCH a REST request to URL "/support/${id}/invalid" with payload
"""json
{
