Feature: Tests the events Service using a REST client.
 
  Scenario: Save the events first.
    ...
    And I POST a REST request to URL "/events" with payload
    """
    {
  ...
  Scenario: Retrieve the saved events
    Given that "entity" equals "events"
    And I construct a REST request with header "x-chenile-tenant-id" and value "tenant0"
    And I GET a REST request to URL "/${entity}/${id}"
    Then success is true
    And the REST response key "id" is "${id}"

  Scenario: Save a events using an ID that already is determined
