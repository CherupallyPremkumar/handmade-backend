package com.handmade.ecommerce.onboarding.domain.aggregate;

import com.handmade.ecommerce.onboarding.domain.enums.VerificationStatus;
import com.handmade.ecommerce.onboarding.domain.valueobject.BusinessDetails;
...
/**
 * Seller Onboarding Case Aggregate Root
 * Manages the seller onboarding process lifecycle and state transitions.
 * This represents a "Case" or "Application" for becoming a seller.
 */
@Entity
@Table(name = "hm_seller_onboarding_case", uniqueConstraints = {
                @UniqueConstraint(name = "uk_onb_email_country_active", columnNames = { "email", "country",
                                "is_active" })
}, indexes = {
                @Index(name = "idx_onb_email_country", columnList = "email,country"),
                @Index(name = "idx_onb_state", columnList = "state")
})
@Getter
@Setter
