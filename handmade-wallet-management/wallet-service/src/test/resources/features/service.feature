Feature: Tests the wallet Service using a REST client.
 
  Scenario: Save the wallet first.
    ...
    And I POST a REST request to URL "/wallet" with payload
    """
    {
  ...
  Scenario: Retrieve the saved wallet
    Given that "entity" equals "wallet"
    When I construct a REST request with authorization header in realm "tenant0" for user "t0-premium" and password "t0-premium"
  ...
  Scenario: Save a wallet using an ID that already is determined
