Feature: Tests the localization Service using a REST client.
 
  Scenario: Save the localization first.
    ...
    And I POST a REST request to URL "/localization" with payload
    """
    {
  ...
  Scenario: Retrieve the saved localization
    Given that "entity" equals "localization"
    And I construct a REST request with header "x-chenile-tenant-id" and value "tenant0"
    And I GET a REST request to URL "/${entity}/${id}"
    Then success is true
    And the REST response key "id" is "${id}"

  Scenario: Save a localization using an ID that already is determined
