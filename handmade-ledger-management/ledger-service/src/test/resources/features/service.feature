Feature: Tests the ledger Service using a REST client.
 
  Scenario: Save the ledger first.
    ...
    And I POST a REST request to URL "/ledger" with payload
    """
    {
  ...
  Scenario: Retrieve the saved ledger
    Given that "entity" equals "ledger"
    When I construct a REST request with authorization header in realm "tenant0" for user "t0-premium" and password "t0-premium"
  ...
  Scenario: Save a ledger using an ID that already is determined
