Feature: Tests the razorpay Service using a REST client.
 
  Scenario: Save the razorpay first.
    ...
    And I POST a REST request to URL "/razorpay" with payload
    """
    {
  ...
  Scenario: Retrieve the saved razorpay
    Given that "entity" equals "razorpay"
    When I construct a REST request with authorization header in realm "tenant0" for user "t0-premium" and password "t0-premium"
  ...
  Scenario: Save a razorpay using an ID that already is determined
