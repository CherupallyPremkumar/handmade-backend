...
public class CommissionPolicyConfiguration {
    private static final String FLOW_DEFINITION_FILE = "stm/commission-policy-states.xml";
    public static final String PREFIX_FOR_RESOLVER = "commissionPolicy";

    @Bean
    BeanFactoryAdapter commissionPolicyBeanFactoryAdapter() {
        return new SpringBeanFactoryAdapter();
    }

    @Bean
    STMFlowStoreImpl commissionPolicyFlowStore(
            @Qualifier("commissionPolicyBeanFactoryAdapter") BeanFactoryAdapter beanFactoryAdapter) throws Exception {
    ...
    @Bean
    @Autowired
    STM<CommissionPolicy> commissionPolicyEntityStm(
    ...
    @Bean
    @Autowired
    STMActionsInfoProvider commissionPolicyActionsInfoProvider(
    ...
    @Bean
    @Autowired
    DefaultPostSaveHook<CommissionPolicy> commissionPolicyDefaultPostSaveHook(
    ...
    @Bean
    @Autowired
    GenericEntryAction<CommissionPolicy> commissionPolicyEntryAction(
            @Qualifier("commissionPolicyEntityStore") EntityStore<CommissionPolicy> entityStore,
            @Qualifier("commissionPolicyActionsInfoProvider") STMActionsInfoProvider infoProvider,
            @Qualifier("commissionPolicyDefaultPostSaveHook") DefaultPostSaveHook<CommissionPolicy> postSaveHook,
            @Qualifier("commissionPolicyFlowStore") STMFlowStoreImpl stmFlowStore) {
        GenericEntryAction<CommissionPolicy> entryAction = new GenericEntryAction<>(entityStore, infoProvider,
                postSaveHook);
    ...
    @Bean
    XmlFlowReader commissionPolicyFlowReader(@Qualifier("commissionPolicyFlowStore") STMFlowStoreImpl flowStore)
            throws Exception {
    ...
    @Bean
    CommissionPolicyHealthChecker commissionPolicyHealthChecker() {
        return new CommissionPolicyHealthChecker();
    }

    @Bean
    STMTransitionAction<CommissionPolicy> defaultCommissionPolicySTMTransitionAction() {
    ...
    @Bean
    ApproveCommissionPolicyAction commissionPolicyApproveAction() {
        return new ApproveCommissionPolicyAction();
    }

    @Bean
    DeleteCommissionPolicyAction commissionPolicyDeleteAction() {
        return new DeleteCommissionPolicyAction();
    }

    @Bean
    DeprecateCommissionPolicyAction commissionPolicyDeprecateAction() {
        return new DeprecateCommissionPolicyAction();
    }

    @Bean
    AddCommissionRuleAction commissionPolicyAddRuleAction() {
        return new AddCommissionRuleAction();
    }

    @Bean
    UpdateCommissionRuleAction commissionPolicyUpdateRuleAction() {
        return new UpdateCommissionRuleAction();
    }

    @Bean
    RemoveCommissionRuleAction commissionPolicyRemoveRuleAction() {
        return new RemoveCommissionRuleAction();
    }

    @Bean
    @Autowired
    STMTransitionAction<CommissionPolicy> commissionPolicyBaseTransitionAction(
    ...
    @Bean
    ActivityChecker commissionPolicyActivityChecker(
            @Qualifier("commissionPolicyFlowStore") STMFlowStoreImpl stmFlowStore) {
