Feature: Tests the gdprrequest Workflow Service using a REST client. This is done only for the
first testcase. Gdprrequest service exists and is under test.
...
And store "$.payload.mutatedEntity.currentState.stateId" from response to "finalState"
 ...
 Scenario: Send the fail event to the gdprrequest with comments
...
Scenario: Send an invalid event to gdprrequest . This will err out.
When I PATCH a REST request to URL "/gdprrequest/${id}/invalid" with payload
"""json
{
