Feature: Tests the governance Service using a REST client.
 
  Scenario: Save the governance first.
    ...
    And I POST a REST request to URL "/governance" with payload
    """
    {
  ...
  Scenario: Retrieve the saved governance
    Given that "entity" equals "governance"
    And I construct a REST request with header "x-chenile-tenant-id" and value "tenant0"
    And I GET a REST request to URL "/${entity}/${id}"
    Then success is true
    And the REST response key "id" is "${id}"

  Scenario: Save a governance using an ID that already is determined
