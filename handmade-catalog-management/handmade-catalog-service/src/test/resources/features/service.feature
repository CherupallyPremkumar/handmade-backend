Feature: Tests the catalog Service using a REST client.
 
  Scenario: Save the catalog first.
    ...
    And I POST a REST request to URL "/catalog" with payload
    """
    {
  ...
  Scenario: Retrieve the saved catalog
    Given that "entity" equals "catalog"
    And I construct a REST request with header "x-chenile-tenant-id" and value "tenant0"
    And I GET a REST request to URL "/${entity}/${id}"
    Then success is true
    And the REST response key "id" is "${id}"

  Scenario: Save a catalog using an ID that already is determined
