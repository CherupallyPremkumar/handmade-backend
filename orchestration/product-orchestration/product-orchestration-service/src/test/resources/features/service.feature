Feature: Tests the productorch Service using a REST client.
 
  Scenario: Save the productorch first.
    ...
    And I POST a REST request to URL "/productorch" with payload
    """
    {
  ...
  Scenario: Retrieve the saved productorch
    Given that "entity" equals "productorch"
    When I construct a REST request with authorization header in realm "tenant0" for user "t0-premium" and password "t0-premium"
  ...
  Scenario: Save a productorch using an ID that already is determined
