Feature: Testcase ID
  Tests the analytics Workflow Service using a REST client. Analytics service exists and is under test.
...
"""
  ...
  Scenario: Send the archiveMetric event to the analytics with comments
    Given that "comment" equals "Comment for archiveMetric"
    And that "event" equals "archiveMetric"
    When I PATCH a REST request to URL "/analytics/${id}/${event}" with payload
"""json
{
    "comment": "${comment}"
