...
public class PriceConfiguration {
    ...
    @Bean BeanFactoryAdapter priceBeanFactoryAdapter() {
		return new SpringBeanFactoryAdapter();
	}
	
	@Bean STMFlowStoreImpl priceFlowStore(
            @Qualifier("priceBeanFactoryAdapter") BeanFactoryAdapter priceBeanFactoryAdapter
            )throws Exception{
	...
	@Bean @Autowired STM<Price> priceEntityStm(@Qualifier("priceFlowStore") STMFlowStoreImpl stmFlowStore) throws Exception{
		STMImpl<Price> stm = new STMImpl<>();		
		stm.setStmFlowStore(stmFlowStore);
		return stm;
	}
	
	@Bean @Autowired STMActionsInfoProvider priceActionsInfoProvider(@Qualifier("priceFlowStore") STMFlowStoreImpl stmFlowStore) {
		STMActionsInfoProvider provider =  new STMActionsInfoProvider(stmFlowStore);
        WorkflowRegistry.addSTMActionsInfoProvider("price",provider);
        return provider;
	}
	
	@Bean EntityStore<Price> priceEntityStore() {
		return new PriceEntityStore();
	}
	
	@Bean @Autowired StateEntityServiceImpl<Price> _priceStateEntityService_(
    ...
    @Bean @Autowired DefaultPostSaveHook<Price> priceDefaultPostSaveHook(
    ...
    @Bean @Autowired GenericEntryAction<Price> priceEntryAction(@Qualifier("priceEntityStore") EntityStore<Price> entityStore,
    @Qualifier("priceActionsInfoProvider") STMActionsInfoProvider priceInfoProvider,
    ...
    GenericEntryAction<Price> entryAction =  new GenericEntryAction<Price>(entityStore,priceInfoProvider,postSaveHook);
    stmFlowStore.setEntryAction(entryAction);
    return entryAction;
    }

    @Bean @Autowired DefaultAutomaticStateComputation<Price> priceDefaultAutoState(
    @Qualifier("priceTransitionActionResolver") STMTransitionActionResolver stmTransitionActionResolver,
    @Qualifier("priceFlowStore") STMFlowStoreImpl stmFlowStore){
    DefaultAutomaticStateComputation<Price> autoState = new DefaultAutomaticStateComputation<>(stmTransitionActionResolver);
    stmFlowStore.setDefaultAutomaticStateComputation(autoState);
    return autoState;
    }

	@Bean GenericExitAction<Price> priceExitAction(@Qualifier("priceFlowStore") STMFlowStoreImpl stmFlowStore){
    ...
    @Bean STMTransitionAction<Price> defaultpriceSTMTransitionAction() {
    ...
    @Bean @Autowired StmBodyTypeSelector priceBodyTypeSelector(
    @Qualifier("priceActionsInfoProvider") STMActionsInfoProvider priceInfoProvider,
    @Qualifier("priceTransitionActionResolver") STMTransitionActionResolver stmTransitionActionResolver) {
    ...
    @Bean @Autowired STMTransitionAction<Price> priceBaseTransitionAction(
        @Qualifier("priceTransitionActionResolver") STMTransitionActionResolver stmTransitionActionResolver,
    ...
    @Bean ActivityChecker priceActivityChecker(@Qualifier("priceFlowStore") STMFlowStoreImpl stmFlowStore){
        return new ActivityChecker(stmFlowStore);
    }

    @Bean
    AreActivitiesComplete activitiesCompletionCheck(@Qualifier("priceActivityChecker") ActivityChecker activityChecker){
        return new AreActivitiesComplete(activityChecker);
    }

    // Create the specific transition actions here. Make sure that these actions are inheriting from
    // AbstractSTMTransitionMachine (The sample classes provide an example of this). To automatically wire
    // them into the STM use the convention of "price" + eventId + "Action" for the method name. (price is the
    ...
    @Bean RemoveFromSalePriceAction
            priceRemoveFromSaleAction(){
        return new RemoveFromSalePriceAction();
    }

    @Bean PutOnSalePriceAction
            pricePutOnSaleAction(){
        return new PutOnSalePriceAction();
    }

    @Bean ExpirePriceAction
            priceExpireAction(){
        return new ExpirePriceAction();
    }

    @Bean ActivatePriceAction
            priceActivateAction(){
        return new ActivatePriceAction();
    }

    @Bean DeactivatePriceAction
            priceDeactivateAction(){
        return new DeactivatePriceAction();
    }


    @Bean ConfigProviderImpl priceConfigProvider() {
        return new ConfigProviderImpl();
    }

    @Bean ConfigBasedEnablementStrategy priceConfigBasedEnablementStrategy(
    ...
    @Bean @Autowired Function<ChenileExchange, String[]> priceEventAuthoritiesSupplier(
        @Qualifier("priceActionsInfoProvider") STMActionsInfoProvider priceInfoProvider)
                    throws Exception{
    ...
    @Bean ON_SALEPricePostSaveHook
        priceON_SALEPostSaveHook(){
            return new ON_SALEPricePostSaveHook();
    }

    @Bean CREATEDPricePostSaveHook
        priceCREATEDPostSaveHook(){
            return new CREATEDPricePostSaveHook();
    }

    @Bean ACTIVEPricePostSaveHook
        priceACTIVEPostSaveHook(){
            return new ACTIVEPricePostSaveHook();
    }

    @Bean INACTIVEPricePostSaveHook
        priceINACTIVEPostSaveHook(){
            return new INACTIVEPricePostSaveHook();
    }

    @Bean EXPIREDPricePostSaveHook
        priceEXPIREDPostSaveHook(){
