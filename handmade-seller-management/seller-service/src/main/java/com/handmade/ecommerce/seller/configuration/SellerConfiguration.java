...
public class SellerConfiguration {
	...
	@Bean
    @Autowired EntityStore<Seller> sellerEntityStore(SellerRepository sellerRepository) {
    ...
    @Bean @Autowired DefaultPostSaveHook<Seller> sellerDefaultPostSaveHook(
    ...
    GenericEntryAction<Seller> entryAction =  new GenericEntryAction<Seller>(entityStore,sellerInfoProvider,postSaveHook);
    stmFlowStore.setEntryAction(entryAction);
    return entryAction;
    }

    @Bean @Autowired DefaultAutomaticStateComputation<Seller> sellerDefaultAutoState(
    @Qualifier("sellerTransitionActionResolver") STMTransitionActionResolver stmTransitionActionResolver,
    @Qualifier("sellerFlowStore") STMFlowStoreImpl stmFlowStore){
    DefaultAutomaticStateComputation<Seller> autoState = new DefaultAutomaticStateComputation<>(stmTransitionActionResolver);
    ...
    @Bean ActivityChecker sellerActivityChecker(@Qualifier("sellerFlowStore") STMFlowStoreImpl stmFlowStore){
        return new ActivityChecker(stmFlowStore);
    }

    @Bean
    AreActivitiesComplete _activitiesCompletionCheck_(@Qualifier("sellerActivityChecker") ActivityChecker activityChecker){
        return new AreActivitiesComplete(activityChecker);
    }

    // Create the specific transition actions here. Make sure that these actions are inheriting from
    // AbstractSTMTransitionMachine (The sample classes provide an example of this). To automatically wire
    // them into the STM use the convention of "seller" + eventId + "Action" for the method name. (seller is the
    ...
    DeactivateSellerAction
            sellerDeactivateAction(){
        return new DeactivateSellerAction();
    }


    @Bean ConfigProviderImpl sellerConfigProvider() {
        return new ConfigProviderImpl();
    }

    @Bean ConfigBasedEnablementStrategy sellerConfigBasedEnablementStrategy(
    ...
    @Bean @Autowired Function<ChenileExchange, String[]> sellerEventAuthoritiesSupplier(
        @Qualifier("sellerActionsInfoProvider") STMActionsInfoProvider sellerInfoProvider)
                    throws Exception{
    ...
    @Bean
    CREATEDSellerPostSaveHook
        sellerCREATEDPostSaveHook(){
            return new CREATEDSellerPostSaveHook();
    }

    @Bean
    ACTIVESellerPostSaveHook
        sellerACTIVEPostSaveHook(){
            return new ACTIVESellerPostSaveHook();
    }

    @Bean
    SUSPENDEDSellerPostSaveHook
        sellerSUSPENDEDPostSaveHook(){
            return new SUSPENDEDSellerPostSaveHook();
    }

    @Bean
    DELETEDSellerPostSaveHook
        sellerDELETEDPostSaveHook(){
